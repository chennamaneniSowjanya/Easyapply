...
class LinkedinEasyApply:
    ...
    def login(self):
        try:
            ...
            self.browser.find_element('css selector',".btn__primary--large").click()
            time.sleep(random.uniform(5, 10))
    ...
    def apply_jobs(self, location):
        no_jobs_text = ""
        try:
            no_jobs_banner = self.browser.find_element(By.XPATH,"//h1[contains(.,'No matching jobs found.')]")
            no_jobs_text = no_jobs_banner.text
            print('no_job',no_jobs_text)
        except:
            pass
        if 'No matching jobs found' in no_jobs_text:
            print('No matching jobs')
            raise Exception("No matching jobs on this page")

        if 'unfortunately, things aren' in self.browser.page_source.lower():
            raise Exception("No more jobs on this page")

        try:
            job_results = self.browser.find_element('class name',"jobs-search-results-list")
            print(job_results)
            self.scroll_slow(job_results)
            self.scroll_slow(job_results, step=300, reverse=True)

            job_list = self.browser.find_elements('class name','jobs-search-results__list-item')
            print(job_list)
        except:
            raise Exception("No more jobs on this page")
        ...
        for job_tile in job_list:
            ...
            try:
                job_title = job_tile.find_element('class name','job-card-list__title').text
                link = job_tile.find_element('class name','job-card-list__title').get_attribute('href').split('?')[0]
                
            except:
                pass
            try:
                #'job-card-container__company-name',
                company = job_tile.find_element('class name','job-card-container__primary-description').text
            except:
                try:
                 company=job_tile.find_element('class name','job-card-container__company-name').text
                except:
                    traceback.print_exc()
                ...
                pass
            try:
                job_location = job_tile.find_element('class name','job-card-container__metadata-item').text
            except:
                pass
            try:
                apply_method = job_tile.find_element('class name','job-card-container__apply-method').text
            except:
                pass

            print('##############',job_title,link,company,job_location,apply_method)
            ...
            if company.lower() not in [word.lower() for word in self.company_blacklist] and \
               contains_blacklisted_keywords is False and link not in self.seen_jobs:
                try:
                    job_el = job_tile.find_element('class name','job-card-list__title')
                    job_el.click()
    ...
    def apply_to_job(self):
        ...
        try:
            easy_apply_button = self.browser.find_element('class name','jobs-apply-button')
        except:
            return False

        try:
            job_description_area = self.browser.find_element('class name',"jobs-search__job-details--container")
        ...
        while submit_application_text not in button_text.lower():
            ...
            while retries > 0:
                try:
                    self.fill_up()
                    next_button = self.browser.find_element('class name',"artdeco-button--primary")
            ...
            if retries == 0:
                traceback.print_exc()
                self.browser.find_element('class name','artdeco-modal__dismiss').click()
                time.sleep(random.uniform(3, 5))
                self.browser.find_elements('class name','artdeco-modal__confirm-dialog-btn')[1].click()
                time.sleep(random.uniform(3, 5))
                raise Exception("Failed to apply to job!")
        ...
        try:
            ...
            pass
        try:
            self.browser.find_element('class name','artdeco-toast-item__dismiss').click()
            closed_notification = True
        except:
            pass
        time.sleep(random.uniform(3, 5))
    ...
    def home_address(self, element):
        try:
            groups = element.find_elements('class name','jobs-easy-apply-form-section__grouping')
            if len(groups) > 0:
                for group in groups:
                    lb = group.find_element('tag name','label').text.lower()
                    input_field = group.find_element('tag name','input')
                    if 'street' in lb:
    ...
    def additional_questions(self):
        #pdb.set_trace()
        frm_el = self.browser.find_elements('class name','jobs-easy-apply-form-section__grouping')
        if len(frm_el) > 0:
            for el in frm_el:
                # Radio check
                try:
                    radios = el.find_element('class name','jobs-easy-apply-form-element').find_elements('class name','fb-radio')
                ...
                # Questions check
                try:
                    question = el.find_element('class name','jobs-easy-apply-form-element')
                    question_text = question.find_element('class name','fb-form-element-label').text.lower()

                    txt_field_visible = False
                    try:
                        txt_field = question.find_element('class name','fb-single-line-text__input')

                        txt_field_visible = True
                    except:
                        try:
                            txt_field = question.find_element('class name','fb-textarea')
                    ...
                    if txt_field_visible != True:
                        txt_field = question.find_element('class name','multi-line-text__input')
                ...
                # Date Check
                try:
                    date_picker = el.find_element('class name','artdeco-datepicker__input ')
                    date_picker.clear()
                ...
                try:
                    ...
                    dropdown_field = question.find_element('class name','fb-dropdown__select')
                ...
                try:
                    ...
                    clickable_checkbox = question.find_element('tag name','label')
    ...
    def send_resume(self):
        upload_button = self.browser.find_element('css selector','.jobs-document-upload__upload-button')
        input_field=self.browser.find_element('id',"jobs-document-upload-file-input-upload-resume-urn:li:fsu_jobApplicationFileUploadFormElement:urn:li:jobs_applyformcommon_easyApplyFormElement:(3544884931,856805485158140725,document)")
        #self.browser.execute_script("arguments[0].style.display = 'block';", upload_button)
        try:
            self.browser.execute_script("arguments[0].style.display = 'block';", input_field)
        except Exception as e:
            print("JavaScript error:", e)
        upload_button.click()

        #print(upload_button.text.lower())
        time.sleep(7)
        input_field.send_keys(self.resume_dir)
        #file_upload_elements = (By.CSS_SELECTOR, "input[name='file']")
        #print('hiiiiii',self.browser.find_elements(file_upload_elements[0], file_upload_elements[1]))
        """if len(self.browser.find_elements(file_upload_elements[0], file_upload_elements[1])) > 0:
                ...
                for upload_button in input_buttons:
                    ...
                    if 'resume' in upload_type.text.lower():
                        print('resumetest',upload_type.text.lower())"""
    ...
    def radio_select(self, element, label_text, clickLast=False):
        label = element.find_element('tag name','label')
        ...
        if len(frm_el) > 0:
            for el in frm_el:
                ...
                if 'email address' in text:
                    ...
                    print('######',text)
                    """try:
                        country_code_picker = el.find_element('id','text-entity-list-form-component-formElement-urn-li-jobs-applyformcommon-easyApplyFormElement-3459248821-1740051564405995655-phoneNumber-country')
                        print('countrycodepicker',country_code_picker)
                    ...
                    try:
                        phone_number_field = el.find_element('id','single-line-text-form-component-formElement-urn-li-jobs-applyformcommon-easyApplyFormElement-3459248821-1740051564405995655-phoneNumber-nationalNumber')
    ...
    def fill_up(self):
        try:
            easy_apply_content = self.browser.find_element('class name','jobs-easy-apply-content')
            b4 = easy_apply_content.find_element('class name','pb4')
            pb4 = easy_apply_content.find_elements('class name','pb4')
            if len(pb4) > 0:
                for pb in pb4:
                    try:
                        label = pb.find_element('tag name','h3').text.lower()
                        try:
